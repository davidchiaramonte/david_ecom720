view: orders_random {
  ...
  view_label: " I need this to be a pretty long name"
  ...
  dimension: id {
    label: "Id of the order in question"
    primary_key: yes
    type: number
    sql: ${TABLE}.id ;;
  }
  ...
  dimension: form_value {
    label: "Form Value in question"
    type: number
    sql: ${TABLE}.form_value ;;
  }

  dimension: id_embed_repro {
    label: "Risk Rank in question"
    type: number
    sql: ${TABLE}.id ;;
  ...
  dimension_group: created {
    alias: [time_started]
    description: "Time when the customer committed to the trial"
    type: time
    timeframes: [
      raw,
      time,
